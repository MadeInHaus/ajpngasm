...
module.exports = function createFramesNew(files, dir, options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const debug = !options.quiet && !_.isEmpty(options.output);
                    ...
                    stream
                        ...
                        .on('parsed', function () {
                            ...
                            if (data.currPNG && (data.currPNG.width !== this.width || data.currPNG.height !== this.height)) {
                                debug && console.log('[%s] %dx%d, skipped (size mismatch)', filename, this.width, this.height);
                        ...
                        .on('error', err => {
                            stream.removeAllListeners();
                            stream.destroy();
                            debug && console.log(`[${filename}] skipped (${err.message}):`);
                            setImmediate(_getNextPNG);
                        });
                })();
            });
            ...
            return new Promise((resolve, reject) => {
                ...
                if (data.diff.isDefaultImage) {
                    ...
                    if (frames.length === 0) {
                        ...
                        debug && console.log('[%s] %dx%d, skipped (empty diff)', data.currFilename, data.currPNG.width, data.currPNG.height);
                        resolve(data);
                    }
                } else {
