...
module.exports = function createFramesNew(files, dir, options) {
    ...
    return new Promise((resolve, reject) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (data.diff.isDefaultImage) {
                    frames.push({
                        ...
                        png: path.join(process.cwd(), data.currFilename),
                    ...
                    const file = path.join(dir, `frame_${frames.length}`);
                    if (data.currHasAlpha) {
        ...
        function saveManifest(data) {
            return new Promise((resolve, reject) => {
                fs.writeFile(path.join(dir, 'manifest.json'), JSON.stringify(data.manifest, null, 2), err => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(data);
